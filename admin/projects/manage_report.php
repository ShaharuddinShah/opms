...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT r.*, w.name as work_type, e.code as ecode, CONCAT(e.firstname,' ',e.middlename,' ', e.lastname) as fullname FROM `report_list` r inner join `work_type_list` w on r.work_type_id = w.id inner join member_list e on r.member_id = e.id where r.id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
    ...
    $(function(){
        ...
        $('#uni_modal #report-form').submit(function(e){
            ...
            $.ajax({
				...
				error:err=>{
					console.log(err)
					alert_toast("An error occurred1 - Please request submitter to edit",'error');
					end_loader();
				},
