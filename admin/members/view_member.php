...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT *,CONCAT(firstname,' ',middlename,' ', lastname) as fullname FROM `member_list` where id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-auto">
            <img src="<?= validate_image(isset($avatar) ? $avatar : "") ?>" alt="Members Image" class="img-circle border bg-gradient-dark" id="member-img">
        </div>
    </div>
    <div class="row">
        <div class="col-md-6">
            <dl>
                <dt class="text-muted">Members Code</dt>
                <dd class='pl-4 fs-4 fw-bold'><?= isset($code) ? $code : 'N/A' ?></dd>
                <dt class="text-muted">Members Name</dt>
