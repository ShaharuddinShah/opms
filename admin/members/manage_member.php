...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT * FROM `member_list` where id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
    ...
    <form id="member-form" action="" method="post">
        ...
        <div class="row">
            <div class="form-group col-md-4">
                ...
                <small class="mx-2">Member Code</small>
            </div>
        </div>
        <div class="row">
            <div class="form-group col-md-4">
...
<script>
    ...
    function generate_pass(){
        ...
        $('#uni_modal #member-form #password').attr('type','text')
    ...
    $(function(){
        ...
        $('#pass_view').click(function(){
            var type = $('#uni_modal #member-form #password').attr('type')
            if(type == 'password'){
                $('#uni_modal #member-form #password').attr('type','text')
                $(this).html('<i class="fa fa-eye"></i>')
            }else{
                $('#uni_modal #member-form #password').attr('type','password')
