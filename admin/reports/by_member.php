...
<div class="card card-outline card-primary rounded-0 shadow">
	<div class="card-header">
		<h3 class="card-title">Total Rendered Time Per Project By Member</h3>
						...
						<div class="row align-items-end">
							<div class="form-group col-md-4">
                                ...
                                <select name="eid" id="eid" class="form-control form-control-sm select2">
                                    <?php 
                                    $member = $conn->query("SELECT *,CONCAT(firstname,' ',middlename,' ',lastname) as fullname FROM `member_list` order by CONCAT(firstname,' ',middlename,' ',lastname) asc");
        ...
        <div class="container-fluid">
			<div class="row">
				...
				<div class="col-8">
					...
					<h3 class="text-center"><b>Rendered Time By Member Report</b></h3>
			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
					<?php 
						$i = 1;
						$qry = $conn->query("SELECT * from `project_list` where delete_flag = 0 and id in (SELECT project_id FROM report_list where member_id = '{$eid}') order by `name` asc ");
						while($row = $qry->fetch_assoc()):
                            $row['total_duration'] = $conn->query("SELECT SUM(duration) FROM `report_list` where project_id = '{$row['id']}' and member_id = '{$eid}'")->fetch_array()[0];
                            $row['total_report'] = $conn->query("SELECT * FROM `report_list` where project_id = '{$row['id']}' and member_id = '{$eid}'")->num_rows;
					?>
						<tr>
...
<script>
	$(document).ready(function(){
        ...
        $('#filter').submit(function(e){
            e.preventDefault();
            location.href= './?page=reports/by_member&'+$(this).serialize();
        })
       $('#print').click(function(){
		   ...
		   var _h = $('head').clone()
		   var _el = $('<div>')
		   _h.find("title").text("Total Rendered Time Per Project By Member Report - Print View")
